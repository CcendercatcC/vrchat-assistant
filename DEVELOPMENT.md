...
> 本文档面向想 **fork、扩展或向本仓库提交 PR** 的开发者 / AI Agent。
> 阅读顺序建议：README（项目概览）→ AGENTS.md（部署配置）→ 本文档（开发约束）。
> 如果你是 AI Agent：在 fork 后的仓库里动手开发前，请完整阅读本文档，**第 3 节「跨平台约束」是必读**。

## 1. 总体原则

- **fork 自由，自用随意**：本仓库采用 MIT 协议，任何人可以 fork，按自己的需求添加 / 修改功能，无需征得作者同意。自用 fork 想怎么改都行。
...
11. **验证说明**：作者合并前会实际运行验证。PR 描述请写明你的测试方式（可参考 `test-apis.mjs` / `test-websocket.mjs` / `test-ws-direct.mjs`）。

> 目前仓库没有 CI，上述脚本是手动验证工具，合并以作者实际运行结果为准。
...
- **贡献者义务**：涉及 API / WebSocket / 数据库的功能改动，PR 描述必须写明验证方式；能跑现有脚本就跑一遍，不能跑要说明原因。
