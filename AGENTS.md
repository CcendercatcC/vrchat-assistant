...
服务通过 MCP 协议暴露 21 个工具（get_online_friends / get_friend_info / get_friend_events / get_companions / get_online_pattern / get_nicknames / set_nickname / get_mutual_friends / send_friend_request / remove_friend / get_world_name / get_watchlist 等，详见 README），Hermes Agent 可直接调用，无需 curl 手写 JSON-RPC。
